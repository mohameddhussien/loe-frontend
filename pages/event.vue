        ...
        <v-row>
            ...
            <v-col>
                <v-card class="d-flex flex-column" variant="text">
                    <v-card-title>
                        <special-text-design class="text-4xl ma-3" :toBeDecorated="event?.EVENT_NAME"
                            color="bg-pink-accent-2" />
                    </v-card-title>
                    <v-card-subtitle> {{ event?.PRICE }}EGP </v-card-subtitle>
                    <v-card-text>
                    ...
                    <v-card-actions class="flex-wrap gap-2">
                        <my-btn @click="openBookingDetails" prepend-icon="mdi-seat-outline" size="large">Book
                            trip</my-btn>
                        ...
                        <special-text-design class="text-2xl mx-auto" :toBeDecorated="personCounter.toString()"
                            after="People" color="bg-pink-accent-2" />
                    </v-card-actions>
                </v-card>
            </v-col>
        </v-row>
...
const incrementPersonCounter = () => {
    personCounter.value =
        personCounter.value < 10 ? personCounter.value + 1 : personCounter.value;
};
const decrementPersonCounter = () => {
    personCounter.value > 1
        ? personCounter.value--
        : (personCounter.value = personCounter.value);
};
